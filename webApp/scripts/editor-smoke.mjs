...
try {
  ...
  // Floating mode preserves the actual controls, document state and undo stack.
  const normalPaper = await page.locator("#paper-scroll").boundingBox();
  await page.evaluate(() => { window.qaPenButton = document.querySelector("#pen-tool"); });
  await page.locator("#float-tools").click();
  const bubble = page.locator("#floating-tools-button");
  const panel = page.locator("#floating-tools-panel");
  assert(await bubble.isVisible());
  assert(!(await panel.isVisible()));
  assert((await page.locator("#paper-scroll").boundingBox()).height > normalPaper.height + 150);
  assert(await page.evaluate(() => window.qaPenButton === document.querySelector("#pen-tool")));
  const startBubble = await bubble.boundingBox();
  await page.mouse.move(startBubble.x + 28, startBubble.y + 28);
  await page.mouse.down();
  await page.mouse.move(40, 100, { steps: 8 });
  await page.mouse.up();
  assert(!(await panel.isVisible()), "Dragging must not open tools");
  assert((await bubble.boundingBox()).x < 50);
  await bubble.focus();
  const beforeKey = await bubble.boundingBox();
  await page.keyboard.press("ArrowRight");
  assert.equal(Math.round((await bubble.boundingBox()).x - beforeKey.x), 10);
  assert.equal(await page.locator("#page-position").textContent(), "1 / 2", "Moving the circle must not turn pages");
  await page.keyboard.press("Enter");
  assert(await panel.isVisible());
  await page.locator("#highlighter-tool").click();
  assert.match(await bubble.getAttribute("aria-label"), /Highlighter/);
  await page.locator("#toolbar-insert").click();
  assert(await page.locator("#insert-menu").isVisible());
  await page.keyboard.press("Escape");
  assert(await panel.isVisible(), "First Escape closes the nested menu");
  assert(!(await page.locator("#insert-menu").isVisible()));
  await page.keyboard.press("Escape");
  assert(!(await panel.isVisible()));
  assert(await bubble.evaluate(element => document.activeElement === element));
  await bubble.click();
  ...
  assert(await page.locator("#inspector").evaluate(element => element.classList.contains("is-open")));
  ...
  await page.locator("#close-floating-tools").click();
  const drawingSurface = await page.locator("#ink-canvas").boundingBox();
  await page.mouse.move(drawingSurface.x + 150, drawingSurface.y + 180);
  await page.mouse.down();
  await page.mouse.move(drawingSurface.x + 210, drawingSurface.y + 210, { steps: 6 });
  await page.mouse.up();
  await bubble.click();
  assert(await page.locator("#undo-button").isEnabled(), "Writing while collapsed must retain undo history");
  await page.locator("#undo-button").click();
  await page.locator("#close-floating-tools").click();
  // Native touch drag must not write on the page or open the panel.
  const bubbleTouch = await context.newCDPSession(page);
  const touchStart = await bubble.boundingBox();
  await bubbleTouch.send("Input.dispatchTouchEvent", { type: "touchStart", touchPoints: [{ x: touchStart.x + 28, y: touchStart.y + 28 }] });
  await bubbleTouch.send("Input.dispatchTouchEvent", { type: "touchMove", touchPoints: [{ x: 940, y: 800 }] });
  await bubbleTouch.send("Input.dispatchTouchEvent", { type: "touchEnd", touchPoints: [] });
  await bubbleTouch.detach();
  assert(!(await panel.isVisible()));
  assert((await bubble.boundingBox()).x > 800);
  await page.setViewportSize({ width: 390, height: 844 });
  await page.waitForFunction(() => {
    const rect = document.querySelector("#floating-tools-button").getBoundingClientRect();
    return rect.left >= 12 && rect.right <= 378 && rect.bottom <= 832;
  });
  const mobileBubble = await bubble.boundingBox();
  assert(mobileBubble.x >= 12 && mobileBubble.x + 56 <= 378 && mobileBubble.y + 56 <= 832);
  await bubble.click();
  const mobilePanel = await panel.boundingBox();
  assert(mobilePanel.x >= 0 && mobilePanel.x + mobilePanel.width <= 390);
  assert(mobilePanel.y >= 0 && mobilePanel.y + mobilePanel.height <= 844);
  assert(mobilePanel.y + mobilePanel.height <= mobileBubble.y || mobilePanel.y >= mobileBubble.y + 56, "Mobile panel must leave the circle reachable");
  assert(await panel.evaluate(element => element.scrollWidth <= element.clientWidth), "Controls must fit the mobile panel");
  await page.screenshot({ path: `${output}/floating-tools-mobile.png`, animations: "disabled" });
  await page.locator("#close-floating-tools").click();
  await page.setViewportSize({ width: 1024, height: 900 });
  await bubble.click();
  await page.locator("#reset-tools-position").click();
  await page.screenshot({ path: `${output}/floating-tools-desktop.png`, animations: "disabled" });
  await page.locator("#close-floating-tools").click();
  const savedPosition = await bubble.boundingBox();
  await page.reload();
  await page.locator("[data-open-book]").first().click();
  await bubble.waitFor({ state: "visible" });
  assert(await bubble.isVisible());
  assert(!(await panel.isVisible()));
  const restoredPosition = await bubble.boundingBox();
  assert(Math.abs(savedPosition.x - restoredPosition.x) < 1 && Math.abs(savedPosition.y - restoredPosition.y) < 1);
  await bubble.click();
  ...
  assert(!(await panel.isVisible()));
  await bubble.click();
  await page.locator("#dock-tools").click();
  assert(!(await bubble.isVisible()));
  assert(await page.locator("#float-tools").isVisible());
  assert(await page.locator("#pen-tool").evaluate(element => element.closest(".editor-stage") !== null));
